...
class Dictionary:
    ...
    def __local(path: str, extension: str) -> pd.DataFrame:
        ...
        page: list[dict] = [
            {'file': file,
             'vertex': file.rsplit(os.path.basename(path) + os.path.sep, maxsplit=1)[1]}
             for file in files]

        return pd.DataFrame.from_records(page)
    ...
    def exc(self, path: str, extension: str, prefix: str) -> pd.DataFrame:
        ...
        frame = local.assign(key=prefix + local["vertex"],
                             section=local['file'].apply(lambda x: os.path.basename(os.path.dirname(x))))
