...
```shell
docker run --rm -i -t -p 8050:8050 -p 8888:8888
    -w /app --mount type=bind,src="$(pwd)",target=/app 
      -v ~/.aws:/root/.aws fundamentals
```

<br>

Herein, `-p 8888:888` maps the host port `8888` to container port `8888`.  Note, the container's working environment, i.e., -w, must be inline with this project's top directory.  Additionally, visit the links below for more about the flags/options $\rightarrow$
...
The part `-v ~/.aws:/root/.aws` ascertains Amazon Web Services interactions via containers. Get the name of a running instance of ``fundamentals`` via:
...
A GitHub Actions script conducts code analysis within a Cloud GitHub Workspace.  Depending on the script, code analysis may occur `on push` to any repository branch, or `on push` to a specific branch.  The sections herein outline remote code analysis.
...
In aid of consistent group/team developments, amend the `pylintrc` file in line with group/team norms, e.g.,
